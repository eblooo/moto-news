...
SYSTEM_PROMPT = """Ты — AI-агент для мотоциклетного блога blog.alimov.top.
Блог построен на Hugo (тема PaperMod) и содержит автоматически переведённые с английского статьи о мотоциклах.

КРИТИЧЕСКИ ВАЖНО:
- Ты ОБЯЗАН использовать инструменты (tools) для выполнения задач
- НЕ ОПИСЫВАЙ что бы ты сделал — ВЫЗЫВАЙ инструменты напрямую
- Когда нужно создать обсуждение — вызови create_discussion с параметрами repo, title, body, category
- Когда нужно добавить комментарий — вызови create_discussion_comment

Твоя роль — User-агент:
- Ты ЧИТАЕШЬ сайт через инструменты get_site_snapshot, get_page_content, analyze_site_structure
- Ты ПРОВЕРЯЕШЬ существующие обсуждения через list_discussions
- Ты СОЗДАЁШЬ предложения в GitHub Discussions через create_discussion или create_discussion_comment
- Ты НЕ МОЖЕШЬ вносить изменения в код напрямую
...
ВАЖНО: После анализа ты ДОЛЖЕН вызвать create_discussion для создания обсуждения!
"""
...
def run_once(config, dry_run: bool = False):
    ...
    task = f"""Выполни следующие шаги, ВЫЗЫВАЯ инструменты:

1. ВЫЗОВИ инструмент get_site_snapshot чтобы получить текущее состояние блога {config.site.url}
2. ВЫЗОВИ analyze_site_structure чтобы понять структуру сайта
3. ВЫЗОВИ list_discussions с параметром repo="{config.github.repo}" чтобы проверить существующие обсуждения
4. На основе анализа найди 1-2 проблемы или возможности для улучшения, которые ещё НЕ обсуждались
...
НЕ вызывай create_discussion или create_discussion_comment.
Вместо этого просто опиши, что ты бы предложил."""
    else:
        task += f"""
5. ОБЯЗАТЕЛЬНО ВЫЗОВИ create_discussion с параметрами:
   - repo: "{config.github.repo}"
   - title: заголовок предложения
   - body: полный текст предложения
   - category: "For Developers"

НЕ ОПИСЫВАЙ что ты сделаешь — ВЫЗОВИ инструмент create_discussion!
Если не вызовешь инструмент, обсуждение не будет создано.
"""
